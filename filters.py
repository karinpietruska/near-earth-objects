...
class DistanceFilter(AttributeFilter):
    """Filter class for distance attribute."""

    @classmethod
    def get(cls, approach):
        """Class method to get distance attribute."""
        return approach.distance


class DateFilter(AttributeFilter):
    """Filter class for time attribute."""

    @classmethod
    def get(cls, approach):
        """Class method to get date."""
        return approach.time.date()


class VelocityFilter(AttributeFilter):
    """Filter class for velocity attribute."""

    @classmethod
    def get(cls, approach):
        """Class method to get velocity attribute."""
        return approach.velocity


class DiameterFilter(AttributeFilter):
    """Filter class for diameter attribute."""

    @classmethod
    def get(cls, approach):
        """Class method to get diameter attribute."""
        return approach.neo.diameter


class HazardFilter(AttributeFilter):
    """Filter class for hazard attribute."""

    @classmethod
    def get(cls, approach):
        """Class method to get hazardous attribute."""
...
def create_filters(date=None, start_date=None, end_date=None,
    ...
    if date:
        filter_dict['date'] = DateFilter(operator.eq, date)
    if start_date:
        filter_dict['start_date'] = DateFilter(operator.ge, start_date)
    if end_date:
        filter_dict['end_date'] = DateFilter(operator.le, end_date)
    if distance_min:
        filter_dict['distance_min'] = DistanceFilter(operator.ge, distance_min)
    if distance_max:
        filter_dict['distance_max'] = DistanceFilter(operator.le, distance_max)
    if velocity_min:
        filter_dict['velocity_min'] = VelocityFilter(operator.ge, velocity_min)
    if velocity_max:
        filter_dict['velocity_max'] = VelocityFilter(operator.le, velocity_max)
    if diameter_min:
        filter_dict['diameter_min'] = DiameterFilter(operator.ge, diameter_min)
    if diameter_max:
        filter_dict['diameter_max'] = DiameterFilter(operator.le, diameter_max)
    if hazardous is not None:
        filter_dict['hazardous'] = HazardFilter(operator.eq, hazardous)

    filters = tuple(filter_dict.values())
